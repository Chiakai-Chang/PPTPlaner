...
4.  **允許適度擴充 (Allow Reasonable Expansion)**：若備忘稿為了增加深度或解釋清楚，補充了投影片上沒有的背景知識或案例（特別是當原稿較精簡時），只要這些內容與主題相關且不與投影片衝突，應視為優點而非錯誤。
5.  **格式合規性 (Format Adherence)**：備忘稿內容**必須**是純粹的 Markdown 文本，不能被任何形式的程式碼區塊（如 ` ```markdown ... ``` ` 或 ` ``` `）包圍。整個回應必須直接以 `### 🎙️` 開頭。
6.  **路徑洩漏檢查 (Path Leakage Check) [CRITICAL]**:
