...
*   **MECE 原則 (MECE Principle)**：
    *   **Mutually Exclusive (相互獨立)**：確保各頁的主題 (`topic`) 之間沒有重疊。每一頁都應該探討一個獨特的概念或子題，避免在不同頁面重複相同的論點。
    *   **Collectively Exhaustive (完全窮盡)**：確保所有頁面加總起來，能完整涵蓋原始文件 (`source_file`) 的核心內容，不遺漏任何重要觀點。
...
*   **符號安全 (Symbol Safety)**：絕對禁止在 `topic` 中輸出檔案路徑。若原文包含特殊符號如 `@` (e.g., "Pass @k")，請務必使用 **全形符號 `＠`** 或反引號包裹 (`` `Pass @k` ``) 以防止路徑洩漏，並保持視覺美觀。

**修正與優化 (Handling Rework)**：
*   若輸入變數中包含 `rework_feedback`，代表你之前的輸出有問題（例如格式錯誤、頁數不符、邏輯缺失）。
