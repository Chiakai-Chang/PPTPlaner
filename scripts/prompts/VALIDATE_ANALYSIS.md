...
5.  **摘要 (summary) 品質**：
    *   **語言檢查**：必須使用**台灣繁體中文 (Taiwanese usage)** 撰寫。
    *   **吸引力**：是否引人入勝，能快速抓住讀者眼球？
    *   **長度限制**：必須嚴格控制在 **50-100 個中文字**之間。
    *   **展示適配**：摘要將作為網頁副標題顯示，內容是否精鍊且不被前端截斷（限 3 行，約 100 字內）？
6.  **總覽 (overview) 品質**：
    *   **語言檢查**：必須使用**台灣繁體中文 (Taiwanese usage)** 撰寫。
    *   **深度**：是否提供了比 `summary` 更深入、涵蓋技術層面的資訊？
    *   **長度限制**：必須控制在 **250-400 個中文字**之間。
    *   **結構**：內容是否連貫、邏輯清晰？是否涵蓋了研究背景、方法與結果？
7.  **內容準確性**：所有提取的元數據和摘要內容，是否忠實地反映了 `source_file` 的內容？
8.  **路徑洩漏檢查 (Path Leakage Check) [CRITICAL]**:
    *   **嚴格檢查**: `project_title`, `summary` 或 `overview` 中是否出現類似 `output\...`, `D:\...`, `.md` 或極長的檔案路徑字串？
    *   **判定**: 若發現，這屬於嚴重錯誤，必須設定 `is_valid: false`。
    *   **反饋**: 在 `feedback` 中嚴厲要求：「移除所有檔案路徑字串，請使用正確的自然語言描述。」
