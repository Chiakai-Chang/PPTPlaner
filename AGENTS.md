...
*   **敘事優先，長度為輔**：你的首要目標是創造一個流暢、有邏輯、易於吸收的敘事結構。一份標準的簡報長度通常落在 10-25 頁之間，但這只是一個參考。切勿為了湊頁數而犧牲內容的連貫性，或硬塞不相關的資訊。最終的頁數應完全由內容本身的邏輯和深度決定，以「講得順暢、聽得精彩」為最高原則。
...
3.  **正確性**：若是引用或節錄原文部分內容，如: 對話、論述、結論、名言等，應同時使用原文的語種、數據及表達方式，再補上恰當的翻譯，減少認知差異，必須原文與翻譯都有，不能只有原文沒有翻譯，也不能只有翻譯沒有原文。
...
5.  **總覽 (overview) 品質**：
    ...
    *   長度是否大致在 150-200 字之間？
    *   內容是否連貫、邏輯清晰？
6.  **內容準確性**：所有提取的元數據和摘要內容，是否忠實地反映了 `source_file` 的內容？
