...
**輸出**: 一個純 JSON 物件，格式如下:
```json
{"is_valid": true/false, "feedback": "若 is_valid 為 false，在此提供宏觀的、可執行的修改建議。例如：'整份簡報的語氣不一致，前三頁較為隨意，後五頁突然變得非常學術，請統一為學術風格。' 或 '第五頁和第六頁的邏輯跳躍太大，建議在兩者之間增加一頁過渡，或調整第五頁的結尾以引導至第六頁。'"}
```
